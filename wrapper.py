...
def brute(url):
    ...
    if ('WordPress' in cms):
        ...
        url = urllib.parse.urljoin(url, '/') + "user/login"
        print(url)
        response = requests.get(url, headers=headers)
        if(response.status_code == 200):
            filepass = open(wrapper_config.PASSW_FILE, "r")
        ...
        url = urllib.parse.urljoin(url, '/') + "/admin/index.php"
        print(url)
        response = requests.get(url, headers=headers)
        if(response.status_code == 200):
            filepass = open(wrapper_config.PASSW_FILE, "r")
...
def sites_dev(*objects, sep=' ', end='\n', file=sys.stdout):
    if wrapper_config.Check_List == True:
        print('Check list target')
        output = []
        urls = open(wrapper_config.URLS_FILE, "r", encoding="utf-8").read().splitlines()
        for url in urls:
